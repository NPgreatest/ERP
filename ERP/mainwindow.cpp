...
void MainWindow::on_calculate_clicked()
{
    ...
    for(int i=0;i<output.size();i++){
        ...
        model_out->setItem(i,4,new QStandardItem(output[i].Get_Date().toString()));
        model_out->setItem(i,5,new QStandardItem(output[i].Get_Date().addDays(-output[i].Get_ahead()).toString()));
    }
}
...
void MainWindow::Import_Data()
{
    ...
    query3=new QSqlQuery();
        ...
        while(query->next()){
            ...
            if(way=="采购"){
                mm=method::purchase;
                process.Insert_Part(partNo,name,unit,mm,lossrate,beforedate,QVector<detail>());
            }else{
                mm=method::product;
                QString child_no;
